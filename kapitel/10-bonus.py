...
# 2. Schritt: Addiert die beiden Werte und speichert das Ergebnis in einer weiteren Variablen (ergebnis_add). Gebt das Ergebnis per print auf der Konsole aus.

# 3. Schritt: Fügt drei weitere Variablen (ergebnis_sub, ergebnis_mul und ergebnis_div) hinzu. Speichert das Ergebnis der Subtraktion (-), das Ergebnis der Multiplikation (*) und das Ergebnis der Division (/) in der jeweiligen Variablen. Fügt nun auch die passenden print-Befehle hinzu.

# 4. Schritt: Extra-Challenge: Passt die print-Befehle so an, dass auf der Konsole, nicht nur das Ergebnis, sondern die gesamte Rechnung zu sehen ist. Zum Beispiel so: 3 * 4 = 12
# Macht das für jede Rechenoperation
